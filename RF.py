...
# 创建随机森林
def random_forest(train, ratio, n_features, max_depth, min_size, n_trees):
    trees = []
    for i in range(n_trees):
        train1 = get_subsample(train, ratio)  # 从切割的数据集中选取子集
        tree = build_tree(train1, n_features, max_depth, min_size)
        # print 'tree %d: '%i,tree
        trees.append(tree)
...
def servable(actual_row):
    ...
    for fold in folds:
        ...
        predict_value = bagging_predict(random_forest(train_set, ratio, n_features, max_depth, min_size, n_trees), actual_row)
        predict_values.append(predict_value)
    return max(set(predict_values), key=predict_values.count)
